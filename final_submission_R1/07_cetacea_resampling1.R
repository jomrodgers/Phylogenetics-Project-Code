...
#Re-sampling approach no. 1 to more thoroughly investigate underlying patterns of trait evolution
#The cetacean dataset is much larger than the one for ichthyosaurs, hence we will re-sample to match N

#Do results change when the cetacean dataset is downsampled randomly?
...
for (i in 1: iterations)try({
  ...
  cet.data_resampled <- cdat01[sample(nrow(cdat01), 48), ]
  
  #match data and tree
  comp <- match.phylo.data(cphy01, cet.data_resampled)
  phy02 <- comp$phy
  dat02 <- comp$data
  ...
  OUModel <- fitContinuous(tree, skull, model="OU", bounds=list(alpha=c(0,exp(5)))) #increased bounds
  EBModel <- fitContinuous(tree, skull, model="EB", bounds=list(a=c(log(10^(-5))/max(node.depth.edgelength(tree)),-1e-16))) #increased bounds
  trendModel <- fitContinuous(tree, skull, model="trend", bounds=list(slope = c(min = -500, max = 3000))) #increased bounds
...
write.csv(aicc.scores, "aicc_cetaceans_resampled.csv")
write.csv(akaike.weights, "aw_cetaceans_resampled.csv")
write.csv(BM_parameters, "bm_cetaceans_resampled.csv")
write.csv(OU_parameters, "OU_cetaceans_resampled.csv")
write.csv(EB_parameters, "EB_cetaceans_resampled.csv")
write.csv(trend_parameters, "trend_cetaceans_resampled.csv")
write.csv(drift_parameters, "drift_cetaceans_resampled.csv")

#plotting Akaike weights
#next two lines read in results from previous runs!
#akaike.weights <- read.csv("aw_cetaceans.csv") #in case one later returns to the model fitting output
...
pdf("Fig_S11_C_Akaike_weights_cetaceans_100_resamplings.pdf", 8, 6)
