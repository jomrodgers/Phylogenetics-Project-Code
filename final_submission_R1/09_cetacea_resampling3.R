...
#Re-sampling approach no. 3 to more thoroughly investigate underlying patterns of trait evolution
...
#Do results change when filter feeders are excluded?
...
#checking time spans and branch lengths for comparative purposes (not needed for model fitting)
#timespans <- ages[,1]-ages[,2]
#  range(timespans)
#  mean(timespans)
#phy <- DatePhylo(phy02, ages, method="equal", rlen=1)
...
#loop model fitting over entire tree set, taxa (no filter feeders) the same for all iterations
#trees
#skull
...
for (i in 1: iterations)try({
  ...
  OUModel <- fitContinuous(trees[[i]], skull, model="OU", bounds=list(alpha=c(0,exp(5)))) #increased bounds
...
write.csv(aicc.scores, "aicc_cetaceans_nofilter.csv")
write.csv(akaike.weights, "aw_cetaceans_nofilter.csv")
write.csv(BM_parameters, "bm_cetaceans_nofilter.csv")
write.csv(OU_parameters, "OU_cetaceans_nofilter.csv")
write.csv(EB_parameters, "EB_cetaceans_nofilter.csv")
write.csv(trend_parameters, "trend_cetaceans_nofilter.csv")
write.csv(drift_parameters, "drift_cetaceans_nofilter.csv")

#plotting Akaike weights
#next two lines read in results from previous runs!
#akaike.weights <- read.csv("aw_cetaceans_nofilter.csv") #in case one later returns to the model fitting output
...
trend_para <- read.csv("trend_cetaceans_nofilter.csv")
hist(trend_para$slope, breaks=100)

density.data <- melt(akaike.weights)
pdf("Fig_S11_E_Akaike_weights_cetaceans_nofilterfeeders_100iterations.pdf", 8, 6)
