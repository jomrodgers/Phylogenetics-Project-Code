...
#Re-sampling approach no. 2 to more thoroughly investigate underlying patterns of trait evolution
#The cetacean dataset consists of both extant and extinct lineages, a different compostion than the ichtyosaur data

#Do results change when extant lineages are excluded?
...
#match data and tree
comp <- match.phylo.data(phy00_fossil, cdat01)
phy02 <- comp$phy
...
for (i in 1: iterations)try({
  ...
  EBModel <- fitContinuous(trees[[i]], skull, model="EB", bounds=list(a=c(log(10^(-5))/max(node.depth.edgelength(trees[[i]])),-1e-16))) #a seems to be at bounds occasionally hence the increased parameter bounds
...
write.csv(aicc.scores, "aicc_cetaceans_fossils.csv")
write.csv(akaike.weights, "aw_cetaceans_fossils.csv")
write.csv(BM_parameters, "bm_cetaceans_fossils.csv")
write.csv(OU_parameters, "OU_cetaceans_fossils.csv")
write.csv(EB_parameters, "EB_cetaceans_fossils.csv")
write.csv(trend_parameters, "trend_cetaceans_fossils.csv")
write.csv(drift_parameters, "drift_cetaceans_fossils.csv")

#plotting Akaike weights
#next two lines read in results from previous runs!
akaike.weights <- read.csv("aw_cetaceans_fossils.csv") #in case one later returns to the model fitting output
...
trend_para <- read.csv("trend_cetaceans_fossils.csv")
hist(trend_para$slope, breaks=100)
mean(trend_para$slope)

mean(akaike.weights$EB)+mean(akaike.weights$trend)

density.data <- melt(akaike.weights)
pdf("Fig_S11_D_Akaike_weights_cetaceans_100_fossils.pdf", 8, 6)
