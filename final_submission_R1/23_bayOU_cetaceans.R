...
cphy01 <- ccomp$phy
cdat4bayou <- ccomp$data
cdat4bayou[,2:5] <- apply(cdat4bayou[,2:5], 2, as.numeric)
cdat4bayou <- cdat4bayou[cphy01$tip.label,]

cages <- cbind(FAD=cdat4bayou$FAD, LAD=cdat4bayou$LAD)
rownames(cages) <- cdat4bayou$Taxon
...
size <- data.frame(taxon=as.character(cdat4bayou[,1]), 
                   size=as.numeric(cdat4bayou[,2]))
...
write.csv(out$statistics, "./cbayou_results/bayOU_a_stats.csv")
write.csv(out$branch.posteriors, "./cbayou_results/bayOU_a_posteriors.csv")



#plots

pdf("./cbayou_results/bayOU_regimes_a.pdf")
par(mfrow=c(1,1))
...
pdf("./cbayou_results/bayOU_heatmap_a.pdf")
par(mfrow=c(1,1))
plotBranchHeatMap(phy, chain, "theta", burnin = 0.3, edge.width=4, pal = hcl.colors, show.tip.label=F)
dev.off()
...
plotBranchHeatMap(phy, chain2, "theta", burnin = 0.3, edge.width=4, pal = hcl.colors, show.tip.label=F)
#phenogram.density(phy, trait, burnin = 0.3, chain2, pp.cutoff = 0.3, pal = cm.colors)


write.csv(out2$statistics, "./cbayou_results/bayOU_b_stats.csv")
write.csv(out2$branch.posteriors, "./cbayou_results/bayOU_b_posteriors.csv")


#plots

pdf("./cbayou_results/bayOU_regimes_b.pdf")
par(mfrow=c(1,1))
...
pdf("./cbayou_results/bayOU_heatmap_b.pdf")
par(mfrow=c(1,1))
plotBranchHeatMap(phy, chain2, "theta", edge.width=4, burnin = 0.3, pal = hcl.colors, show.tip.label=F)
dev.off()
...
pdf("./cbayou_results/bayOU_posterior_diagnostics.pdf", useDingbats=F)
par(mfrow=c(1,1))
...
pdf("./cbayou_results/bayOU_pp_prior_a.pdf")
par(mfrow=c(1,1))
...
pdf("./cbayou_results/bayOU_pp_prior_b.pdf")
par(mfrow=c(1,1))
